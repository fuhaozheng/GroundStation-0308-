...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.ViewHolder> implements View.OnClickListener{
    private List<TaskofAirpoint> datas;
    public MessageAdapter(List<TaskofAirpoint> datas) {
        this.datas = datas;
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder,  int position) {
        viewHolder.mTextView.setText(datas.get(position).getMessage()+":"+datas.get(position).getParameter());
    ...
    public void onClick(View v) {
        if (mOnItemClickListener != null) {
            //注意这里使用getTag方法获取数据
            mOnItemClickListener.onItemClick(v,datas.indexOf((TaskofAirpoint)v.getTag()));
        }
    }
